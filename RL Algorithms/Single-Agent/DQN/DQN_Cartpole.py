...
class DQN:
    def __init__(self, env, per = False):
        ...
        self.model = Q_Net(self.obs_dim, self.num_actions)
        self.optimizer = torch.optim.Adam(self.model.parameters(), 0.01)

        self.target_model = Q_Net(self.obs_dim, self.num_actions)
        ...
        for i in range(training_steps):
            ...
            for i in range(self.num_updates):
                ...
                for i in range(len(observations)):
                    ...
                    loss = F.mse_loss(estimated_q_value, target_q_value)
        ...
        for i in range(self.rollouts_per_batch):
            ...
            while(not done and not truncated and t < self.max_timesteps):
                if random.random() < self.epsilon_decay():
                    ...
                    # print("not random")
                    action = np.argmax(self.model(torch.from_numpy(obs)).detach().numpy())
