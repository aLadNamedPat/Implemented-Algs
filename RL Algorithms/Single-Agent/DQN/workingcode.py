...
class Agent(object):
    ...
    #Used to generate the training data that is then used to train the brain
    def find_targets_per(self, batch):
        batch_len = len(batch)

        states = np.array([o[1][0] for o in batch]) #Saving the states and next states as numpy arrays
        states_ = np.array([o[1][3] for o in batch])

        p = self.brain.predict(states) #Predict what the value of the current state is
        p_ = self.brain.predict(states_) #Predict what the value of the next state is
        pTarget_ = self.brain.predict(states_, target=True) #Predict what the value of the next state is using the target neural network

        x = np.zeros((batch_len, self.state_size)) #Create an empty array that is the size of the batch_sizes and the states to save
        y = np.zeros((batch_len, self.action_size))
        errors = np.zeros(batch_len) #Create an array filled with zeros containing the error between the estimated value and the actual (approximated) value
        ...
        for i in range(batch_len):
            ...
            if done:
                ...
                if self.target_type == 'DDQN':
                    t[a] = r + self.gamma * pTarget_[i][np.argmax(p_[i])] #Generate the estimated value of the next state based on the target that is approximated
